pro mesma_v6, case_number, in_file, $ 
...
for i=0, nx do begin
 if i mod 100 eq 0 then print, strcompress('Running line '+string(i+1)+' of '+string(nx+1)) ; should be column?
  ...
  temp=where(pixel eq float(null)/float(refl_scale))   ;jump out null pixel
...
;  if keyword_set(lls_sum_to_one) eq 1 then do_write_spectral_image_v6, out, indgen(n_em)+1, orig_lib, models, out_file, /strip_last_band $
